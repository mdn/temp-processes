...
The objective of this document is to have a single place with all the information in the above listed pages, collect all recent guideline updates discussed in various meetings, and also include some expectations from PR authors as discussed in "Saved Replies" doc.
This information will eventually be part of community contributor docs.
...
- **Day-to-day content improvement work:** documentation of new APIs, new CSS properties, significant platform updates and content additions.
...
- **Minor fixes:** small updates done to the site to fix typos, grammatical issues, and technical inaccuracies. These issues are usually found by users of MDN Web Docs.
- **Content bug fixes:** usually done by volunteers to close issues on `mdn/content` repo.

Regardless of how content changes are done, they will be submitted as pull requests on GitHub.
To ensure that the site does not get out-of-date, timely reviews and merging PRs are managed as follows:
...
3. **Content update** is done via a site rebuild once every 24 hours.
...
- **File an issue describing the intended change.**
  When you submit a PR, this issue should be referenced using `#` followed by the issue's ID, for example `#1234`.
- **PRs should be short and focused to one issue** or a related set of changes to submit together.
...
After you have opened a PR to make changes on the MDN Web Docs:
...
These guidelines apply to anyone who is assigned with MDN content PR reviews.
...
- Familiarize yourself with the MDN Writing style guide, and use it to inform your reviews of new text content.
  - When reviewing a PR, use GitHub's review tools. Use "Request changes" when submitting a review that will require the PR author to do some more work, or "Approve" if the submission is ready to add and you want to merge it. Reviewing proposed changes in a pull request (on Github) is also useful if you want more information.
    Review guidelines
    (source: https://github.com/mdn/content/blob/main/REVIEWING.md#review-guidelines)
- Follow code of conduct: Be polite and constructive at all times when writing review comments or otherwise interacting with the PR author and other community members. We are all bound by our Code of Conduct when contributing to MDN Web Docs, which means adhering to Mozilla's Community Participation Guidelines. If anyone has engaged in behavior that is potentially illegal or makes you or someone else feel unsafe, unwelcome, or uncomfortable, you are encouraged to report it. We want MDN Web Docs to be a welcoming, friendly community that we can all be proud of.
  - Limit the scope of review: If a PR is fine in itself, is a clear improvement to the content, and makes the change it claims to make in the description, you should, as a rule, merge it, even if you see other improvements that could be made in the same file. If you want to ensure that these other improvements will be taken care of, file a follow-up issue or a PR of your own to address these improvements/issues.

### Reviewing a pull request

- **Add a starting comment** as soon as possible that you are aware of the PR and will start the review soon. This is an important step to avoid someone else reviewing the PR at the same time as you and so that others know it's on your radar for review.
...
Reach out to someone else to ask for help, like a colleague or someone else in your group of topic review owners (if you know who they are).
If you are not sure who to approach for help, then ping our `@core-yari-content` group to ask for help.
...
Assigning reviewers:
A person is automatically assigned to review an open PR. If the author has someone specific in mind that they would like to review their work (e.g. you are working within a team) the author should assign/unassign relevant reviewers
...
- **Don't merge unless 'depends on'** PRs are merged
